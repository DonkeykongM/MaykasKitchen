...
export const Newsletter = () => {
  ...
  return (
    <section className="bg-gradient-to-br from-purple-600 via-blue-600 to-purple-700 text-white py-16 relative overflow-hidden">
      {/* Decorative pattern */}
      ...
      <div className="container mx-auto px-4 text-center relative z-10">
        ...
        <div className="max-w-xl mx-auto">
          {submitStatus === 'success' ? (
            <div className="bg-white/20 backdrop-blur-sm rounded-lg p-6 flex flex-col items-center justify-center animate-fadeIn border border-white/30">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-grow">
                <input 
                  ...
                  className={`w-full px-4 py-3 rounded-lg border border-white/30 text-purple-800 focus:outline-none focus:ring-2 focus:ring-white/50 bg-white/95 backdrop-blur-sm ${
                    submitStatus === 'error' ? 'border-red-300 bg-red-50/95' : ''
                  }`}
                  aria-label="Din e-postadress"
                  disabled={submitStatus === 'submitting'}
                />
                {submitStatus === 'error' && (
                  <div className="absolute -bottom-6 left-0 text-xs text-red-200 bg-red-600/80 px-2 py-1 rounded-sm backdrop-blur-sm">
                    {errorMessage}
                  </div>
                )}
              </div>
              <button 
                type="submit"
                disabled={submitStatus === 'submitting'}
                className="bg-white/90 text-purple-700 px-6 py-3 rounded-lg hover:bg-white transition-all font-semibold flex items-center justify-center group whitespace-nowrap disabled:opacity-70 shadow-lg"
              >
